...
# libbrotlidec / harfbuzz / harfbuzz-subset: upstream's ENABLE_BROTLI and ENABLE_HARFBUZZ default ON,
# so libpoppler.a now contains BrotliStream.cc.o and FontSubsetter.cc.o. We link the harnesses
# ourselves and BUILD_SHARED_LIBS=OFF, so a static archive carries no dependency info — the final
# link must name these or ld fails with "undefined reference to symbol
# 'BrotliDecoderDecompressStream' ... DSO missing from command line". These are exactly the
# pkg-config names upstream declares in PC_REQUIRES_PRIVATE (CMakeLists.txt:515, :589).
