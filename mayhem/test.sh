...
# NOTE: this DEPS list is duplicated from mayhem/build.sh — keep them in sync. The golden oracle
# links libpoppler.a the same way the harnesses do, so it needs the same libs: upstream's
# ENABLE_BROTLI/ENABLE_HARFBUZZ default ON, libpoppler.a contains BrotliStream.cc.o, and with
# BUILD_SHARED_LIBS=OFF the archive carries no dependency info. Omitting them fails the oracle
# link with "undefined reference to symbol 'BrotliDecoderDecompressStream' ... DSO missing from
# command line" — which fails the IMAGE BUILD, since the Dockerfile runs test.sh.
DEPS="freetype2 lcms2 libopenjp2 fontconfig libpng libtiff-4 libbrotlidec harfbuzz harfbuzz-subset"
